...
async def create_member(member_data: MemberCreate):
    ...
    phone = member_data.phone_number.strip() if member_data.phone_number else ""
    
    existing = await db.members.find_one({
        "$or": [
            # Same name + surname (ignore case)
            {
                "first_name": {"$regex": f"^{member_data.first_name}$", "$options": "i"},
                "surname": {"$regex": f"^{member_data.surname}$", "$options": "i"}
            },
            # Same phone number
            {"phone_number": phone} if phone else {"_id": "impossible"}
        ]
    })
    ...
    if existing:
        raise HTTPException(
            status_code=400, 
            detail="A member with this name or phone number already exists!"
        )
    ...
    try:
        ...
        logger.error(f"Auto-sync after create failed: {e}")
...
async def update_member(member_id: str, member_data: MemberUpdate):
    ...
    try:
        ...
        logger.error(f"Auto-sync after update failed: {e}")
    return member
...
async def delete_member(member_id: str):
    ...
    try:
        ...
        logger.error(f"Auto-sync after delete failed: {e}")
...
async def save_attendance(attendance_data: AttendanceCreate):
    ...
    try:
        ...
        logger.error(f"Auto-sync attendance failed: {e}")
